...
class Cell:
    ...
    def __str__(self):
        return f'Результат операции {self.quanity * "*"}'
    def __add__(self, other):
        return Cell(self.quanity + other.quanity)
    def __sub__(self, other):
        return self.quanity - other.quanity if (self.quanity - other.quanity) > 0 else print('Отрицательно!')

    def __mul__(self, other):
        return Cell(int(self.quanity * other.quanity))
    def __trudiv__(self, other):
        return Cell(round(self.quanity // other.quanity))

    def make_order(self, cell_in_row):
            row = ''
            for i in range(int(self.quanity / cell_in_row)):
                row += f'{"*" * cell_in_row} \\n'
            row += f'{"*" * (self.quanity % cell_in_row)}'
            return row
cells1 = Cell(3)
cells2 = Cell(9)
print(cells1)
