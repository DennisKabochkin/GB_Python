...
class StoreMashines:
    ...
    def reception(self):
        ...
        print(f'Выход - Q, продолжить - Enter')
        q = input(f'далее:' )
        if q == 'q' or q == 'Q':
...
class Printers(StoreMashines):
    def to_print(self):
        return f'to print smth {self.number} times'
class Scanners(StoreMashines):
    def to_scan(self):
        return f'to scan smth {self.number} times'
class Copiers(StoreMashines):
    def to_cop(self):
        return f'to cop smth {self.number} times'

unit_1 = Printers('hp', 'LaserJet 1022', 2, 100, 1)
unit_2 = Scanners('canon', 4300, 3, 150, 2)
unit_3 = Copiers('Xerox', 1100, 5, 200, 3)
